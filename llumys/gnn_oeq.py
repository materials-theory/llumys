...
# from ase.geometry import get_distances # we will use get_distances_torch instead of this
...
# troubleshooting for lower version pytorch: ex. 2.6
import torch
if not hasattr(torch.library, "register_autocast"):
    def _noop(*args, **kwargs): pass
    torch.library.register_autocast = _noop

import openequivariance as oeq
from openequivariance import Irrep, Irreps, TensorProduct, TensorProductConv

from e3nn.o3 import spherical_harmonics, Linear, FullyConnectedTensorProduct # Irrep, Irreps, TensorProduct -> OEQ / FullyConnectedTensorProduct -> Defined
...
def print_memory_usage():
    ...
    process = psutil.Process(os.getpid())  # bring current process ID
...
class RadialFunction(nn.Module):
    ...
    def forward(self, r: torch.Tensor) -> torch.Tensor:
        """
        r: [E], edge distance
        return: [E, n_basis], raw radial basis corresponds to R(r_ij)
        """
...
#         super().__init__()
#         self.internal_weights = True # put external weights to TensorProduct
...
#         instr = [
#             (i_1, i_2, i_out, "uvw", True) # connection mode fixed :D
...
#         _problem = oeq.TPProblem(
#             irreps_in1, irreps_in2, irreps_out, instr,
#             shared_weights = True,
#             internal_weights = False,
#             irrep_dtype  = np.float64 if dtype.lower()=="float64" else np.float32,
#             weight_dtype = np.float64 if dtype.lower()=="float64" else np.float32
#             )
#         self._tp = TensorProduct(_problem, torch_op = torch_op)

#         self.weight_numel = int(self._tp.weight_numel)
#         # self.weight = nn.Parameter(torch.randn(self.weight_numel, dtype=self.dt)) # e3nn의 TensorProduct에서 internal weight는 이렇게 되던데?
#         with torch.no_grad():
#             self.weight = nn.Parameter(torch.empty(self.weight_numel, dtype=self.dt))
...
#                 path_weight=1.0,
#                 path_shape=(self.irreps_in1[i_in1].mul, self.irreps_in2[i_in2].mul, self.irreps_out[i_out].mul),
#             )
#             for i_in1, i_in2, i_out, connection_mode, has_weight in instr
#         ]

#     def forward(self, x, y):
#         return self._tp(x, y, self.weight)


#     # # 이것도 apply로 바꾸는 게 나을지도?
#     # def to(self, *args, **kwargs):
#     #     device, dtype, non_blocking, convert_to_format = torch._C._nn._parse_to(*args, **kwargs)
        
#     #     if dtype is not None and self.weight.dtype != dtype:
#     #         self.weight.data = self.weight.data.to(dtype=dtype)
#     #     if device is not None and self.weight.device != device:
#     #         self.weight.data = self.weight.data.to(device=device)
#     #     super().to(*args, **kwargs)
#     #     # if device is not None or dtype is not None:
#     #     #     self._tp = self._tp.to(*args, **kwargs)
#     #     return self

#     def _apply(self, fn, recurse=True):
#         '''
#         To avoid self._tp recreation
#         '''
#         tp_backup = self._tp if hasattr(self, '_tp') else None
#         if tp_backup is not None:
#             delattr(self, '_tp')
...
class MessagePassing(nn.Module):
    ...
    def __init__(
        ...
        self.tp = TensorProductConv(_problem, torch_op=True, deterministic=True)
    ...
    def forward(self, data: dict) -> dict:
        ...
        recv_sorted, recv_perm = torch.sort(edge_j)
        Y_ij_sorted = edge_attrs[recv_perm]
        w_sorted = w[recv_perm]
        send_sorted = edge_i[recv_perm]
        _, send_perm = torch.sort(send_sorted)
        x_out = self.tp.forward(x, Y_ij_sorted, w_sorted, recv_sorted, send_sorted, send_perm)
    ...
    def _apply(self, fn, recurse=True):
        '''
        To avoid self.tp recreation
        '''
        tp_backup = self.tp if hasattr(self, 'tp') else None
        if tp_backup is not None:
            delattr(self, 'tp')
        super()._apply(fn, recurse=recurse)
        if tp_backup is not None:
